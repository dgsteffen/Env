...
;;
;; 2.  Place the following line in your .emacs file :
;;    (require 'slashdot)
;;
;; 3.  Fill in values for slashdot-username, and slashdot-password,
;;    etc, if necessary.
;;
;; 4.  M-x slashdot will then give you the latest headlines from Slashdot.
;;
;;
;;
;;; Version History:
...
;;
;;    0.0.7   Fixed indentation, etc.  Forced the mode to update
;;           the &lt; characters, etc.
;;
...
(defvar slashdot-username "seanr"
  "Username to use when retrieving the slashdot headlines.")

(defvar slashdot-password "wskier."
  ...
  (let ((tcp-connection))
    ...
    (let ((request (concat "GET " file " HTTP/1.0\n")))
	  ...
	  (progn
	    ...
	    (message "Cookie : %s" slashdot--cookie)
